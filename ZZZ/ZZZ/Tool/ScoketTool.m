...
@interface ScoketTool()<SRWebSocketDelegate>
{
    int _index;
    NSTimer * heartBeat;
...
-(void)SRWebSocketOpen{
    ...
    self.socket = [[SRWebSocket alloc] initWithURLRequest:
                   [NSURLRequest requestWithURL:[NSURL URLWithString:@"http://13.114.27.163:5202/socket.io/?EIO=3&transport=websocket"]]];//这里填写你服务器的地址
...
- (void)initHeartBeat
{
    dispatch_main_async_safe(^{
        ...
        heartBeat = [NSTimer timerWithTimeInterval:25 target:self selector:@selector(sentheart) userInfo:nil repeats:YES];
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    if (webSocket == self.socket) {
        NSLog(@"************************** socket 连接成功************************** ");
        if (self.delegate) {
            [self.delegate SRWebSocketDidOpen];
        }
    }
}
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    ...
    if (webSocket == self.socket || self.socket == nil) {
        NSLog(@"************************** socket连接断开************************** ");
        NSLog(@"被关闭连接，code:%ld,reason:%@,wasClean:%d",(long)code,reason,wasClean);
        [self SRWebSocketClose];
        if (self.delegate) {
            [self.delegate SRWebSocketDidClose];
        }
    }
}
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message  {
    ...
    if (webSocket == self.socket) {
        ...
        if (self.delegate) {
            [self.delegate SRWebSocketDidReceiveMsg:str];
        }
